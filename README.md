...
Modifying companion HTML versions directly is discouraged and instead install the following pre-commit hook which will generate them during committing your change on .ipynb files:
```
git config --local core.hooksPath git-config/hooks
```
Make sure the specific version of `nbconvert` is installed. If not, run:
```
pip install ipython~=5.10.0 nbconvert~=5.6.1
```
